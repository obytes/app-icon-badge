...
import { createEnvBadge } from './create-env-badge';
import { createVersionBadge } from './create-version-badge';
...
export async function addBadge({ icon, environment, version }: Params) {
  let resultImage = await Jimp.read(icon);

  //Create the environment banner image
  const environmentBadge = await createEnvBadge({ environment });
  ...
  const versionBadge = await createVersionBadge({ version });
